...
local function GET_MUTE()
    cmd = "pactl get-sink-mute $(pactl get-default-sink)"
...
local function TOG_VOLUME_CMD(device) return 'pactl set-sink-mute @DEFAULT_SINK@ toggle' end
...
local function worker(user_args)
    ...
    watch("/bin/sh -c 'pactl get-sink-volume $(pactl get-default-sink) | awk \"{print \\$5}\" | sed \"s/.$//\"'",
        refresh_rate,
