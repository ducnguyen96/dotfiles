...
function Volume:new(args)
	...
	obj.widget:set_image(config .. "/awesome/icons/volume/shared/0.png")
...
function Volume:update(status)
	...
	self.widget:set_image(config .. "/awesome/icons/volume/shared/" .. img .. ".png")
end
...
function Volume:getVolume()
	...
	-- 	result = run("pactl list sinks | grep '^[[:space:]]Volume:' | head -n $(( " ..
	-- 		(self.device + 1) .. " )) | tail -n 1 | sed -e 's,.* \\([0-9][0-9]*\\)%.*,\\1,'")
	-- 	return result
	-- end
	-- return string.gsub(string.match(result, "%[%d*%%%]"), "%D", "")
end
