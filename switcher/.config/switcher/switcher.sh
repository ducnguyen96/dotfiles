...
function get_windows_info() {
    windows=$(hyprctl clients -j | jq 'map(select(.mapped == true and .class != "com.switcher.hypr")) | sort_by(.focusHistoryID) | .[] | {class, address, title, workspace}' | jq -s '.')
...
function move_to_current_workspace(){
    ...
    hyprctl dispatch movetoworkspacesilent $current_workspace,$switcher_id
    ...
    if [[ "$activ e_class" != "$switcher_id" ]]; then
        if [[ -z "$switcher_instance" ]] && [[ "${#icons[@]}" -gt 0 ]]; then
            spawn
        # else
        #     hyprctl dispatch focuswindow $switcher_id
        fi
